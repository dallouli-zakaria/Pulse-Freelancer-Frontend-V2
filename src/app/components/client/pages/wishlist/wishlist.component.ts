...
export class WishlistComponent implements OnInit {
  ...
  index() {
    this.wishListService
      .getFavoriteFreelancersdetails(this.clientId)
      .subscribe({
        next: (data: any) => {
    ...
    this.wishListService.getFavoriteFreelancers(this.clientId).subscribe({
      next: (data: any) => {
        this.favoriteFreelancers = new Set(
          data.map((item: any) => item.freelancer_id)
        );
  ...
  toggleFavorite(freelancerId: number) {
    if (this.favoriteFreelancers.has(freelancerId)) {
      this.wishListService
        .removeFromWishlist(this.clientId, freelancerId)
        .subscribe(() => {
  ...
  onSearch() {
    ...
    this.totalPages = Math.ceil(
      this.filteredFreelancers.length / this.itemsPerPage
    );
    this.updatePagination();
  }
  ...
  updatePagination() {
    ...
    const endIndex = Math.min(
      ...
      this.filteredFreelancers.length
    );
    this.paginatedFreelancers = this.filteredFreelancers.slice(
      startIndex,
      endIndex
    );
    this.totalPages = Math.ceil(
