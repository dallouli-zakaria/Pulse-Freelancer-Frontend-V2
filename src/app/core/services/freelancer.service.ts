...
export class FreelancerService {
  ...
  private readonly subjectBe: BehaviorSubject<Freelancer[]> =
    new BehaviorSubject<Freelancer[]>([]);
  ...
  public index() {
    this.freelancer = this.http
      .get(`${this.url}/${Constant.FREELANCERS}`)
      .pipe(shareReplay(1))
      .subscribe({
        next: (data: any) => {
          this.subjectBe.next(data);
        },
  ...
  public fetchPaginatedFreelancers(page: number = 1): void {
    ...
    this.http
      .get<PaginatedResponse<Freelancer>>(`${this.url}/freelancerPagination`, {
        params,
      })
      .pipe(shareReplay(1))
      .subscribe({
  ...
  public update(id: any, data: Partial<Freelancer>): Observable<Freelancer[]> {
    this.freelancer = this.http.put<Freelancer>(
      `${this.url}/${Constant.FREELANCERS}/${id}`,
      data
    );
    return this.freelancer;
  }
  ...
  public delete(id: any): Observable<Freelancer> {
    this.freelancer = this.http.delete(
      `${this.url}/${Constant.FREELANCERS}/${id}`
    );
    return this.freelancer;
  }
  ...
  public show(id: any) {
    this.freelancer = this.http
      .get(`${this.url}/${Constant.FREELANCERS}/${id}`)
      .subscribe({
        next: (data: any) => {
          this.subjectBe.next(data);
          console.log(data);
  ...
  addSkillToFreelancer(freelancerId: number, skillData: any): Observable<any> {
    return this.http
      .post<any>(`${this.url}/freelancers/${freelancerId}/skills`, skillData)
  ...
  updateSkillLevel(
    ...
    return this.http
      .put<any>(`${this.url}/freelancers/${freelancerId}/skills/${skillId}`, {
        level,
      })
      .pipe(catchError(this.handleError));
  }
  ...
  removeSkillFromFreelancer(
    ...
    return this.http
      .delete<any>(`${this.url}/freelancers/${freelancerId}/skills/${skillId}`)
      .pipe(catchError(this.handleError));
  }
  ...
  //get matched skill score with post
  public getScore(freelancerId: number, postId: number): Observable<any> {
    this.score = this.http.get<any>(
      `${this.url}/${Constant.FREELANCERS}/${freelancerId}/posts/${postId}/skills-match-score`
    );
    return this.score;
  }
