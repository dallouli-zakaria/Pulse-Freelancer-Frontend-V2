...
export class PostsService {
  ...
  public fetchPaginatedPosts(page: number = 1) {
    ...
    this.post = this.http.get<PaginatedResponse<Post>>(
      `${this.url}/postPagination`,
      { params }
    );
    return this.post;
  }
  ...
  public update(id: any, data: any): Observable<Post> {
    this.post = this.http.put<Post>(
      `${this.url}/${Constant.POSTS}/${id}`,
      data
    );
  ...
  public addpost(data: any, id: number): Observable<Post> {
    this.post = this.http.post<Post>(
      `${this.url}/${Constant.POSTS}/${id}/posts`,
      data
    );
  ...
  public showbclient(id: any) {
    this.post = this.http.get<Post[]>(
      `${this.url}/${Constant.POSTS}/client/${id}`
    );
    return this.post;
  }
  ...
  public showbpost(id: any) {
    this.post = this.http.get<Post>(
      `${this.url}/${Constant.POSTS}/post_id/${id}`
    );
    return this.post;
  }

  // Check if a freelancer has made an offer on a post
  public checkFreelancerOffer(
    postId: number,
    freelancerId: number
  ): Observable<{ offer_exists: boolean }> {
  ...
  public getClosedPostsByFreelancerId(freelancerId: number) {
    this.post = this.http.get<any>(
      `${this.url}/posts/closed-by-freelancer/${freelancerId}`
    );
    return this.post;
  }
  ...
  public showbfreelancer(id: any) {
    this.post = this.http.get<Post[]>(
      `${this.url}/${Constant.POSTS}/freelancer/${id}`
    );
    return this.post;
  }
