...
export class WishListService {
  ...
  getFavoriteFreelancersdetails(id: number): Observable<any> {
    this.freelancer = this.http.get<any>(
      `${this.url}/wishlist/client/${id}/freelancers`
    );
  ...
  addToWishlist(client_id: number, freelancer_id: number): Observable<any> {
    return this.http.get<any>(
      `${this.url}/wishlist/add/${client_id}/${freelancer_id}`
    );
  }
  ...
  removeFromWishlist(
    ...
    return this.http.delete<any>(
      `${this.url}/wishlist/remove/${client_id}/${freelancer_id}`
    );
  }
}
